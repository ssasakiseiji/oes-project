...
export class ObservationUnitsService {
  ...
  async getStudents(observationUnitId: number) {
    const assignments = await this.prisma.observationUnitAssignment.findMany({
      where: { observationUnitId, user: { roles: { has: 'student' } } },
      include: { user: true },
  ...
  create(name: string, address: string) {
    return this.prisma.observationUnit.create({ data: { name, address } });
  }

  async update(id: number, name: string, address: string) {
    try {
      return await this.prisma.observationUnit.update({
        where: { id },
        data: { name, address },
      });
    } catch (error) {
