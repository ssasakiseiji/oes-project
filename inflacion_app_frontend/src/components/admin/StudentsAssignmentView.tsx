...
import type { Commerce, StudentWithAssignments, StudentsWithAssignmentsResponse } from '../../types/api';
...
export const StudentsAssignmentView = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const data = await apiFetch<StudentsWithAssignmentsResponse>('/api/commerce-assignments/students');
    ...
    const handleAddCommerce = async (commerceId: number) => {
        ...
        try {
            ...
            const updatedData = await apiFetch<StudentsWithAssignmentsResponse>('/api/commerce-assignments/students');
            ...
            if (getErrorStatus(err) === 409) {
                ...
                toast.error(`Error al asignar comercio: ${getErrorMessage(err)}`);
            }
        } finally {
            setIsSaving(false);
        }
    ...
    const handleRemoveCommerce = async (commerceId: number) => {
        if (!selectedStudent) return;
