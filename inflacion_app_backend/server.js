    ...
    try {
        ...
        if (prices && Object.keys(prices).length > 0) {
            ...
            if (priceEntries.length > 0) {
                const insertValues = priceEntries.map(([productId, price]) => `(${userId}, ${productId}, ${commerceId}, ${periodId}, '${price}')`).join(',');
                const insertQuery = `INSERT INTO draft_prices (user_id, product_id, commerce_id, period_id, price) VALUES ${insertValues}`;
